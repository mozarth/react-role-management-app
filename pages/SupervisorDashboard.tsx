...
import { 
  Bell, MapPin, Clock, Camera, FileText, QrCode, LogOut, AlertTriangle, Home 
} from 'lucide-react';
      ...
      <nav className="sticky bottom-0 bg-background border-t pt-2 pb-6">
        <div className="grid grid-cols-4 gap-1 px-2">
          ...
          <Button 
            ...
            className="flex flex-col items-center h-auto py-2" 
            onClick={() => setLocation('/mobile/notifications')}
            className={`flex flex-col items-center h-auto py-2 ${unreadCount > 0 ? 'text-primary' : ''}`}
          >
            <Bell className="h-4 w-4 mb-1" />
            <span className="text-xs">Notificaciones</span>
            {unreadCount > 0 && (
              <span className="absolute top-1 right-8 h-2 w-2 rounded-full bg-red-500"></span>
            )}
          </Button>
        </div>
      </nav>
